...
export default function UpdateEmail() {
  ...
  const [stage, setStage] = useState(
    () => localStorage.getItem("updateEmailStage") || "request",
  ); // 'request' | 'verify'
  ...
  const handleSendCode = useCallback(
    ...
    [canSend, user?.email, dispatch, startCountdown],
  );
  ...
  const handleConfirmChange = useCallback(
    async (e) => {
      ...
      try {
        ...
        toast.success("Email updated successfully. Please log in.");
    ...
    [otpValid, otpString, newEmail, dispatch],
  );
  ...
  if (!user) {
    return (
      <div className="card card--cozy">
        <p className="form-hint">You must be logged in to update your email.</p>
      </div>
    );
  }
      ...
      <header className="section-head">
        <div className="section-titles">
          ...
          <p className="section-sub">
            We&apos;ll verify the change using a 6-digit code. Current email:{" "}
      ...
      {stage === "request" && (
        <form className="form-grid-2" onSubmit={handleSendCode}>
          <label className="form-field">
            ...
            {newEmail === user.email && (
              <div className="form-error">This is your current email</div>
            )}
          </label>
          ...
          <div className="inline-actions form-actions-right">
            <button className="btn btn-primary" disabled={!canSend || loading}>
      ...
      {stage === "verify" && (
        <form className="form-grid-1" onSubmit={handleConfirmChange}>
          ...
          >
            ...
            <button className="btn btn-primary" disabled={!otpValid || loading}>
