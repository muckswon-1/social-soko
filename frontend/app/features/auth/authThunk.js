    ...
    async(_,{rejectWithValue}) => {
         try {
          ...
          return ok({ user: data.user });
            ...
            return rejectWithValue(
                fail(error, 'Failed to refresh user')
            )
         }
    }
)
...
export const updatePassword = createAsyncThunk(
    ...
    async({password, token}, {rejectWithValue}) => {
        try {
             const { data } = await api.post(`/auth/reset-password/${token}`, { password: password });

             return ok({ message: data?.message || 'Password reset successful' });

        } catch (error) {
            return rejectWithValue(
                fail(error,'Failed to reset password')
            )
        }
    }
)
...
export const sendSixDigitCode = createAsyncThunk(
    ...
    async({email}, {rejectWithValue}) => {
        try {
            ...
            return rejectWithValue(
                fail(error, 'Could not send verification code.')
            )
        }
    }
...
export const resetPasswordWithDigitCode = createAsyncThunk(
    ...
    async({ password, digitCodes}, {rejectWithValue}) => {
        ...
        try {
             const response = await api.post('/auth/reset-password-with-digit-code',{password, digitCodes});
    ...
    async({email}, {rejectWithValue}) => {
        try {
            const response = await api.post('/auth/send-verification-email', { email });
             ...
             return ok({message: response?.data?.message || 'Verification email sent to your email'})

        }catch(error){

            return rejectWithValue(
                fail(error, 'Could not send verification email.')
...
export const updateEmailWithDigitCode = createAsyncThunk(
    ...
    async({email, digitCodes}, {rejectWithValue}) => {
        try {
            const response = await api.post('/auth/email-update-with-digit-code', {email, digitCodes});

            return ok({message: response?.data?.message || 'Email updated successfully'})

        }catch(error){
            return rejectWithValue(
                fail(error, 'Error updating email')
            )
        }
    }
)
...
export const verifyEmail = createAsyncThunk(
    ...
    async({token}, {rejectWithValue}) => {
      ...
      return ok({ message: data?.message || 'Email verified successfully' });
        ...
        return rejectWithValue(
            fail(error, 'Email verification failed')
        )
       }
